...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FIELDS = [
      ...
      { key: 'priority', group: '通知外观', label: '优先级（需要你操作时）', type: 'select', options: PRIORITIES, hint: '结果类固定用 default，不弹横幅打断你' },
      ...
      { key: 'headsUp', label: '悬浮通知', type: 'switch', hint: '「需要你操作」的通知用重要性 HIGH 的通道弹横幅；结果类不弹，避免打断' },
      { key: 'headsUpChannel', label: '悬浮通道', type: 'text', hint: '通道 id；改成一个新值（如 dsh-heads-up2）会建一个全新通道，用来修复被调低过重要性的旧通道' },
