...
function createNotifier(config, deps = {}) {
  ...
  function ensureChannel(payload, options = {}) {
    ...
    if (payload?.actionable !== true || !cfg.headsUp || cfg.backend !== 'termux') return Promise.resolve(undefined)
  ...
  function speak(payload) {
    ...
    args.push(text)
    // 下限给到 30 秒：TTS 引擎首次加载/合成可能很慢（实测某些机型要十几秒）
    const timeoutMs = Math.min(90000, Math.max(30000, text.length * 600))
  ...
  async function deliver(payload) {
    ...
    if (cfg.backend === 'command') {
      payload.notificationTag = tagFor(payload, false)
  ...
  async function runEnvironmentCheck(options = {}) {
    ...
    if (cfg.headsUp && cfg.backend === 'termux') await ensureChannel({ actionable: true }, { force: true })
