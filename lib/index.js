...
const DEFAULTS = {
  ...
  /** 通知标题前缀。 */
  titlePrefix: 'DSH',
  /** `termux` = 用 termux-notification；`command` = 用自定义 shell 命令模板。 */
  backend: 'termux',
  /** backend=command 时的命令模板，支持 {title} {content} {tag} 占位符（自动做 shell 转义）。 */
  command: '',
...
function normalizeConfig(raw) {
  ...
  cfg.titlePrefix = pickString(source.titlePrefix, DEFAULTS.titlePrefix)
  ...
  cfg.command = typeof source.command === 'string' ? source.command : DEFAULTS.command
  cfg.tapUrl = typeof source.tapUrl === 'string' ? source.tapUrl : DEFAULTS.tapUrl
  cfg.backend = source.backend === 'command' ? 'command' : 'termux'
...
const SettingsSchema = z.object({
  ...
  titlePrefix: z.any().default(DEFAULTS.titlePrefix),
  backend: z.any().default(DEFAULTS.backend),
  command: z.any().default(DEFAULTS.command),
...
function defaultExec(file, args, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (timedOut) {
        finish(new Error(`termux-notification 在 ${timeoutMs}ms 内没有返回（通常是 Termux:API 应用未安装）`))
        return
      }
      if (code !== 0) {
...
function createNotifier(config, deps = {}) {
  ...
  function buildTermuxArgs(payload, plan, channel) {
    const args = [
      '--id', `${cfg.notificationId}-${payload.tag}`,
      '--title', payload.title,
      '--content', payload.content,
    ]
    if (cfg.priority) args.push('--priority', cfg.priority)
    if (cfg.group) args.push('--group', cfg.group)
  ...
  function ensureChannel() {
    if (!cfg.headsUp || cfg.backend !== 'termux') return Promise.resolve(undefined)
    if (state.channelReady) return Promise.resolve(CHANNEL_ID)
  ...
  function renderVoiceText(payload) {
    ...
    const values = { title: payload.title, content: payload.content }
    return template.replace(/\{(title|content)\}/g, (_match, key) => values[key] ?? '').replace(/\s+/g, ' ').trim().slice(0, 300)
  }
  ...
  function speak(payload) {
    ...
    args.push(text)
    const timeoutMs = Math.min(60000, Math.max(15000, text.length * 400))
    Promise.resolve()
      .then(() => exec(bin, args, { timeoutMs }))
      .catch((error) => {
        if (!state.warnedVoiceFailure) {
          state.warnedVoiceFailure = true
          emit('warn', `语音播报失败（通知照常发出）：${error?.message ?? error}`)
        }
      })
  }
  ...
  /** 把 payload 组装成 backend=command 的 shell 脚本（占位符单趟替换 + 转义）。 */
  function commandScript(payload) {
    const template = cfg.command.trim()
    if (!template) throw new Error('backend=command 但没有配置 command')
    // 单趟替换：避免某一字段的值里含有别的占位符而被二次替换。
    const values = { title: payload.title, content: payload.content, tag: payload.tag }
    return template.replace(/\{(title|content|tag)\}/g, (_match, key) => shellQuote(values[key]))
  }
  ...
  async function deliver(payload) {
    ...
    if (cfg.backend === 'command') {
      const job = Promise.resolve().then(() => exec('sh', ['-c', commandScript(payload)], { timeoutMs: cfg.execTimeoutMs }))
      fireVibrate(plan)
      return job
    }
    const bin = findBin()
  ...
  function countFailure(error) {
    ...
    if (!state.warnedFailure) {
      ...
      emit('warn', `发送通知失败：${message}。${missingCommand ? '请先执行 `pkg install termux-api`。' : ''}若未安装 Termux:API 应用，请从 F-Droid 或 GitHub Releases 安装「Termux:API」；也可把 backend 改成 command 走自己的通道`)
    }
  ...
  function send(payload) {
    try {
      ...
      state.lastAt = at
      if (cfg.dryRun) {
        emit('info', `[dry-run] ${payload.title} :: ${payload.content.replace(/\n+/g, ' | ')}`)
        state.sent += 1
        return true
      }
      if (cfg.backend === 'command' && !cfg.command.trim()) {
        if (!state.warnedCommand) {
          state.warnedCommand = true
          emit('warn', 'backend=command 但没有配置 command，通知已跳过')
        }
        return false
      }
  ...
  async function runEnvironmentCheck(options = {}) {
    ...
    else step('config', '插件总开关', 'ok', `已启用，通道 ${cfg.backend}`)

    if (cfg.backend === 'command') {
      step('channel', '自定义命令通道', cfg.command.trim() ? 'ok' : 'fail',
        cfg.command.trim() || '没有配置 command',
        cfg.command.trim() ? undefined : '填写「自定义命令模板」，或把通道改回 termux')
    } else {
      // 2) 命令是否存在
      const bin = findBin()
      ...
      if (!cfg.headsUp) {
        ...
        step('headsUp', '悬浮通知', 'ok', `专用通道 ${CHANNEL_ID}（重要性 HIGH）「${CHANNEL_NAME}」`,
          '若仍不弹横幅，检查系统里 Termux:API 的「悬浮通知/横幅通知」权限，以及是否在勿扰模式')
      }
    }

    // 4d) 语音通知（与通道无关，两种 backend 都适用）
    if (!cfg.voice) {
      ...
      const spoken = renderVoiceText({ title: `${cfg.titlePrefix} · 需要你选择`, content: '（结果摘要）' })
      const detail = `将念出：${spoken}（${lang === '' ? '语言跟随系统' : `语言 ${lang}`}）`
      if (engines.includes('label') || engines.includes('"name"')) {
        step('voice', '语音通知', 'ok', detail, '播报走 NOTIFICATION 音频流；系统静音时通常不出声')
      } else {
        step('voice', '语音通知', 'warn', `${detail} —— 但没检测到可用的 TTS 引擎`,
          '去系统设置 → 语言与输入/无障碍 → 文字转语音，安装或选择一个引擎；也可以把「播报语言」留空')
      }
    }
    ...
    if (sendTest) {
      try {
        ...
        step('send', '试发一条通知', 'fail', error?.message ?? String(error),
          '若是超时或挂起，通常是 Termux:API 应用没装好、或被系统限制了后台运行；也可以把通道改成 command 转发到 ntfy 等')
      }
    }
  ...
  function questionContent(questions) {
    ...
    for (const question of questions.slice(0, 3)) {
      const header = typeof question?.header === 'string' && question.header.trim() ? `${question.header.trim()}：` : ''
      let line = `${header}${typeof question?.question === 'string' ? question.question.trim() : ''}`.trim()
      const options = Array.isArray(question?.options)
        ? question.options.map((option) => option?.label).filter((label) => typeof label === 'string' && label.trim())
        : []
      if (options.length > 0) {
        line += `\n选项：${options.slice(0, 4).join(' / ')}${options.length > 4 ? ' …' : ''}`
      }
      if (line) lines.push(line)
    }
    if (questions.length > 3) lines.push(`…还有 ${questions.length - 3} 个问题`)
  ...
  function onTurnEnd(session, sessionId, data) {
    ...
    if (title) lines.push(flat(title, 80))
    if (cfg.includeSnippet) {
      const snippet = snippets.get(sessionId)
      if (snippet) lines.push(flat(snippet, cfg.snippetChars))
    }
    if (kind === 'error') {
      const detail = reason.error?.message ?? reason.error?.code
      if (typeof detail === 'string' && detail) lines.push(flat(`错误：${detail}`, 160))
    }
    if (elapsed !== undefined && elapsed >= 1000) lines.push(`用时 ${formatDuration(elapsed)}`)
    send({
      tag: 'turn',
      title: `${cfg.titlePrefix} · ${TURN_TITLES[kind] ?? '轮次结束'}`,
      content: lines.join('\n') || `会话 ${sessionId.slice(0, 8)} 已结束`,
    })
  }
  ...
  return {
      ...
      try {
        if (cfg.notifyOnQuestion) {
          ...
          if (questions.length > 0) {
            send({ tag: 'question', title: `${cfg.titlePrefix} · 需要你选择`, content: questionContent(questions) })
          }
        }
      } catch (error) {
      ...
      try {
        if (cfg.notifyOnApproval) {
          ...
          const reason = typeof request?.reason === 'string'
            ? request.reason
            : request?.reason !== undefined ? JSON.stringify(request.reason) : ''
          send({
            tag: 'approval',
            title: `${cfg.titlePrefix} · 需要授权`,
            content: flat(`${tool} 请求授权${reason ? `：${reason}` : ''}`, 300),
          })
        }
      } catch (error) {
    ...
    startupCheck() {
      ...
      if (cfg.backend !== 'termux') {
        if (!cfg.command.trim()) emit('warn', 'backend=command 但没有配置 command，所有通知都会被跳过')
        return
      }
      if (!findBin()) {
...
export { CHANNEL_ID, CHANNEL_NAME, CHECK_ROUTE_PATH, DEFAULTS, SETTINGS_NAMESPACE, SettingsSchema, apply, createNotifier, defaultExec, name, normalizeConfig }
