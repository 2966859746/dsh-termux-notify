...
function createNotifier(config, deps = {}) {
  ...
  function buildTermuxArgs(payload, plan) {
    const args = [
  ...
  function deliver(payload) {
    ...
    const bin = findBin()
    if (!bin) return Promise.reject(new Error('PATH 里找不到 termux-notification（需要 pkg install termux-api）'))
    const job = Promise.resolve().then(() => exec(bin, buildTermuxArgs(payload, plan), { timeoutMs: cfg.execTimeoutMs }))
    fireVibrate(plan)
    return job
  }
...
export { CHECK_ROUTE_PATH, DEFAULTS, SETTINGS_NAMESPACE, SettingsSchema, apply, createNotifier, defaultExec, name, normalizeConfig }
