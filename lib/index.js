...
const DEFAULTS = {
   ...
   *
   * 触发点都是「用户明显已经回到对话里了」：
   *  - 根会话的 `turn/start`（子 agent 的轮次很频繁，不能让它清掉主会话的提醒）；
   *  - `user-questions/request` 落定（**在浏览器里回答提问时那一轮不会重启**，
   *    只看 turn/start 的话，这条提醒会一直赖在通知栏里）；
   *  - `approval/request` 落定（同理）。
   *
   * 只撤销本插件自己发过的 tag，不动别的应用的通知。点过的通知本来就会自动消失
   * （Termux:API 设了 autoCancel），这里清理的是没点过的积压。
   */
  clearStale: true,
...
function normalizeConfig(raw) {
  ...
  cfg.clearStale = pickBool(source.clearStale, DEFAULTS.clearStale)
...
const SettingsSchema = z.object({
  ...
  clearStale: z.any().default(DEFAULTS.clearStale),
...
function createNotifier(config, deps = {}) {
  ...
  const findRemoveBin = typeof deps.findRemoveBin === 'function' ? deps.findRemoveBin : () => findOnPath('termux-notification-remove')
  ...
  /** 记住「我们发过」的 tag，供清空时撤销。只记自己的，不碰别的应用。 */
  function rememberPosted(tag) {
    state.postedTags.push(tag)
    // 上限保护：正常一轮最多几条，这里只是防长跑进程无限增长
    if (state.postedTags.length > 200) state.postedTags.splice(0, state.postedTags.length - 200)
  }

  /**
   * 清空之前发过的通知（best-effort）。
   *
   * `termux-notification-remove <tag>` 就是 `manager.cancel(tag, 0)`，没有批量接口，
   * 所以逐个撤销；单个失败不影响其它，也不会影响 agent 运行。
   * @returns 成功撤销的条数
   */
  async function clearPostedNotifications() {
    // 先等在途的发送落定：否则「发出 → 立刻清空」（例如提问刚发出就被回答）
    // 会因为 tag 还没登记而漏掉这一条。allSettled：单条失败不影响清空其它。
    if (state.inflightPosts.size > 0) await Promise.allSettled([...state.inflightPosts])
    const tags = state.postedTags.splice(0, state.postedTags.length)
    if (tags.length === 0) return 0
    if (state.removeDisabled) return 0
    const bin = findRemoveBin()
    if (bin === undefined) {
      if (!state.warnedRemove) {
        state.warnedRemove = true
        emit('warn', '找不到 termux-notification-remove，无法清空旧通知（修复：pkg install termux-api）')
      }
      return 0
    }
    let cleared = 0
    for (const tag of tags) {
      try {
        await exec(bin, [tag], { timeoutMs: cfg.execTimeoutMs })
        cleared += 1
      } catch (error) {
        if (!state.warnedRemove) {
          state.warnedRemove = true
          emit('warn', `撤销通知失败（不影响新通知发出）：${error?.message ?? error}`)
        }
      }
    }
    return cleared
  ...
  /** 触发一次静默清空：fire-and-forget，绝不阻塞 turn/start。 */
  function scheduleClearPosted() {
    clearPostedNotifications().catch(() => {})
  }
  ...
  function deliver(payload) {
    const delivery = (async () => {
      ...
      fireVibrate(plan)
      await job
      // 只在真的发出去之后才登记，失败的通知无需撤销
      rememberPosted(payload.notificationTag)
    })()
    // 登记「整段投递」而不是里面那一步：deliver 里还有建通道等 await，
    // 只登记 exec 的话，「刚发出就清空」（例如提问发出后立刻被回答）会漏掉这一条。
    state.inflightPosts.add(delivery)
    const done = () => { state.inflightPosts.delete(delivery) }
   ...
   *
   * @param options - `{ sendTest, clear }`：是否真发一条测试通知 / 是否清空旧通知。
  ...
  async function runEnvironmentCheck(options = {}) {
    ...
    // 4e) 清空旧通知：策略 +（可选）本次执行结果
    if (findRemoveBin() === undefined) {
      step('clear', '清空旧通知', 'warn', '找不到 termux-notification-remove',
        '在 Termux 里执行：pkg install termux-api')
    } else if (options.clear === true) {
      const cleared = await clearPostedNotifications()
      step('clear', '清空旧通知', cleared > 0 ? 'ok' : 'skip',
        cleared > 0 ? `已撤销 ${cleared} 条本插件发过的通知` : '当前没有本插件发过的通知需要撤销')
    } else if (cfg.clearStale) {
      step('clear', '清空旧通知', 'ok', '回到对话时清掉过时的：新一轮开始、或你回答完提问/审批',
        '点过的通知本来就会自动消失，这里清的是没点掉的积压；只撤本插件发过的 tag')
    } else {
      step('clear', '清空旧通知', 'skip', '已关闭 —— 旧通知会一直留在通知栏',
        '打开「回到对话时清空过时通知」即可自动清理')
    }
  ...
  /** 是否是子 agent 会话（子会话的轮次很频繁，不能让它清掉主会话的提醒）。 */
  ...
  function onTurnEnd(session, sessionId, data) {
    if (isChildSession(session) && !cfg.notifyChildSessions) return
  ...
  return {
    ...
    onQuestion(request, next) {
      ...
      const answer = typeof next === 'function' ? next() : undefined
      // 无论用户是在通知栏点掉的、还是在浏览器里回答的，这条提醒都已经过期。
      // 注意返回的仍是下游原本的 promise（清空只是搭在旁边的旁路）。
      if (cfg.clearStale) Promise.resolve(answer).then(scheduleClearPosted, scheduleClearPosted)
      return answer
    },
    ...
    onApproval(request, next) {
      ...
      // 决定已经做出（同意/拒绝/取消），提醒过期，顺手清掉。
      if (cfg.clearStale) Promise.resolve(decision).then(scheduleClearPosted, scheduleClearPosted)
      return decision
    },
    ...
    onSessionEvent(session, event) {
      try {
        ...
        if (type === 'turn/start') {
          ...
          // 新一轮开始了，之前那批提醒已经过期 —— 清掉，别让它们堆在通知栏里。
          if (cfg.clearStale && !isChildSession(session)) scheduleClearPosted()
          return
        }
    ...
    /** 清空本插件发过的通知（设置页的「清空旧通知」按钮与新一轮触发都走它）。 */
          ...
          try {
            ...
            try {
              ...
              if (body && body.trim() !== '') {
                ...
                sendTest = parsed?.sendTest === true
                clear = parsed?.clear === true
              }
            } catch {
              /* 空 body / 非 JSON：按「只检测」处理 */
            }
            sendJson(res, 200, await notifier.runEnvironmentCheck({ sendTest, clear }))
          } catch (error) {
