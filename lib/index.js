...
const DEFAULTS = {
   ...
   * 新一轮开始时清空之前发过的通知。
   *
   * 只在**根会话**的 `turn/start` 触发（子 agent 的轮次很频繁，不能让它清掉主会话的提醒）；
   * 只撤销本插件自己发过的 tag，不动别的应用的通知。
   * 点过的通知本来就会自动消失（Termux:API 设了 autoCancel），这里清理的是没点过的积压。
   */
  clearOnNewTurn: true,
...
function normalizeConfig(raw) {
  ...
  cfg.clearOnNewTurn = pickBool(source.clearOnNewTurn, DEFAULTS.clearOnNewTurn)
...
const SettingsSchema = z.object({
  ...
  clearOnNewTurn: z.any().default(DEFAULTS.clearOnNewTurn),
...
function createNotifier(config, deps = {}) {
  ...
  async function deliver(payload) {
    ...
    return job.then(() => { rememberPosted(payload.notificationTag) })
  }
  ...
  async function runEnvironmentCheck(options = {}) {
    ...
    } else if (cfg.clearOnNewTurn) {
      step('clear', '清空旧通知', 'ok', '每轮开始时清掉上一轮的（只撤本插件发过的 tag）',
        '点过的通知本来就会自动消失，这里清的是没点掉的积压')
    } else {
      step('clear', '清空旧通知', 'skip', '已关闭 —— 旧通知会一直留在通知栏',
        '打开「新一轮开始时清空旧通知」即可自动清理')
    }
    ...
    onSessionEvent(session, event) {
      try {
        ...
        if (type === 'turn/start') {
          ...
          if (cfg.clearOnNewTurn && !isChildSession(session)) scheduleClearPosted()
          return
        }
